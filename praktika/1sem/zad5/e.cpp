...
int main() {
    ...
    file.open("teams.dat", ios::in | ios::binary); //ios = Input Output Stream (поток ввода-вывода)
    if (!file.is_open()) {
        cout << "ошибка" << endl;
        return 1;
    }
    ...
    if (file.eof()) {
        file.clear(); // очищаем флаг конца файла
    }
    file.close();*/
    ...
    while (choice != 0) {    
        switch (choice) {
            ...
            case 2: { 
                ...
                file.open("teams.dat", ios::out | ios::app);
                if (!file.is_open()) {
